...
description: "Match bank statement transactions to ledger entries. Flag unmatched items, suggest journal entries for discrepancies, and generate reconciliation reports."
...
Automatically match bank statement transactions to general ledger entries. Flag unmatched items on both sides (bank-only and ledger-only), suggest journal entries for discrepancies, and generate a reconciliation report with the adjusted book balance.

## When to Use

- Bank reconciliation is done manually each month (hours of matching)
- Transactions don't match between bank and ledger (timing differences, missing entries, duplicates)
- No system to flag suspicious or unmatched transactions
- Reconciliation adjustments are entered without documentation

## Capabilities

- Fuzzy matching of bank transactions to ledger entries (amount, date proximity, description)
- Match confidence scoring (exact, high, low, unmatched)
- Unmatched item categorization (deposits in transit, outstanding checks, bank fees, missing entries, duplicates)
- Suggested journal entries for bank-only transactions (fees, interest, corrections)
- Reconciliation summary (adjusted bank balance vs adjusted book balance)
...
### Bank Statement (CSV — from bank export)
```csv
date,amount,description,running_balance
...
2026-06-05,-875.50,"VENDOR PAYMENT ACME CORP",49289.50
2026-06-10,12.50,"INTEREST PAYMENT",49302.00
```

### Ledger Entries (CSV — from QuickBooks/ERP export)
```csv
date,amount,description,account,reference
...
2026-06-05,-875.50,"Acme Corp payment","Cash - Operating","AP-2026-045"
```

## Edge Cases

1. **Timing differences** — Deposit recorded in ledger on June 30, hits bank July 2. Normal — shows as deposit in transit.
2. **Bank fees not in ledger** — Monthly account fee auto-deducted. Suggest journal entry.
3. **Duplicate transactions** — Bank processed same payment twice. Flag as duplicate.
4. **Transposed amounts** — Ledger has $1,250, bank has $1,520. Won't match — flag for review.
5. **Split transactions** — One bank deposit covers 3 client payments in ledger. Need manual matching.
6. **NSF/returned items** — Check bounces. Bank shows reversal. Ledger needs reversal entry.
7. **Multi-currency** — Bank converts at different rate than ledger. Amount won't match exactly — use wider tolerance.

## Integration

- **QuickBooks Online API** — Pull ledger entries directly
- **Bank CSV export** — Standard download from any bank portal
- **Plaid API** — Real-time bank transaction feed (via n8n)
- **n8n workflow** — Auto-trigger reconciliation on first business day of month
- **Trailwise SaaS** — Managed reconciliation dashboard ($49/mo)
