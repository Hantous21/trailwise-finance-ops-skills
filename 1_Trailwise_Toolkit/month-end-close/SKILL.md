...
description: "Automate the month-end close process. Close checklist with task tracking, accrual entries, variance analysis, roll-forward schedules, and sign-off workflow."
...
Structure and automate the monthly close process. Generate a close checklist, prepare standard accrual entries, run variance analysis against budget and prior month, update roll-forward schedules (prepaids, fixed assets, AR allowance), and track sign-offs for each task.

## When to Use

- Month-end close takes 5+ business days
- Same checklist tasks are recreated manually each month
- Accrual entries are estimated from memory instead of data
- Variance explanations are written from scratch every month
- No visibility into close status (who's done, what's pending)

## Capabilities

- Close checklist generation (recurring tasks with owners and due dates)
- Accrual entry suggestions based on historical patterns
- Variance analysis (actual vs budget vs prior month, with materiality filtering)
- Roll-forward schedule updates (prepaids, fixed assets, AR allowance)
- Sign-off tracking (who, when, notes)
- Close progress dashboard (tasks complete, pending, overdue)
...
1. **Prior period still open** — Can't close June if May isn't closed. Block and warn.
2. **Missing accruals** — Controller forgets to accrue a recurring expense. Flag accounts with zero current-month activity but prior 3-month average > threshold.
3. **Intercompany transactions** — Multi-entity close requires coordination. Flag intercompany accounts for dual review.
4. **Year-end adjustments** — December close includes additional tasks (depreciation true-up, inventory count, 1099 prep). Add seasonal tasks.
5. **Weekend/holiday timing** — Due day 3 means 3rd business day, not calendar day. Adjust for holidays.

## Integration

- **QuickBooks Online** — Pull trial balance, post accrual entries
